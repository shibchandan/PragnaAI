...
bool rejectDisallowedOrigin(const httplib::Request& req, httplib::Response& res) {
...
int main() {
    ...
    svr.Post("/doc/ask", [&](const httplib::Request& req, httplib::Response& res) {
        ...
        if (hits.empty()) {
            std::ostringstream ss;
            ss << "{\"answer\":"
               << jS("I couldn't find enough relevant information in your indexed documents to answer that confidently.")
               << ",\"model\":\"\""
               << ",\"grounded\":false"
               << ",\"contexts\":[]"
               << ",\"docCount\":" << docDB.size() << '}';
        ...
        std::string prompt =
            ...
            "If the excerpts do not contain enough information, reply exactly with: "
            "\"I don't have enough information in the indexed documents to answer that.\" "
            "Do not use outside knowledge, and do not mention excerpts, retrieval, or context in your reply.\n\n"
